...
def test_for_bleu():
    ...
    folder_path = "https://lnsigo.mipt.ru/export/serikov/mmodal-ac/clotho_subset/"
...
#добавление колонки preds из результатов работы модели
    ...
    #выделили и сохранили в ту же таблицу
    df['pred'] = predictions
    ...
    #расчет метрики блю
    for row in csv.DictReader(open(df_path), fieldnames=columns):
        lens = []
        for col in columns:
    ...
    return("BLEU metric value is", sum(avgs)/len(avgs))
