...
{ time docker-compose -f docker-compose.yml \
    ...
    -f assistant_dists/dream_ranking_and_midas_based_dm/dev.yml up --detach --build --force-recreate; } 2> time_output.txt

echo "Service is up"
sleep 5
real_time=$(grep "real" time_output.txt | awk '{print $2}')

python response_selectors/ranking_and_intent_based_response_selector/test_launch_time.py "$real_time"
