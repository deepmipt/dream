...
url = "http://0.0.0.0:8082/respond"

current_directory = os.path.dirname(__file__)
file_path = os.path.join(current_directory, 'test_sample.json')


def test_input_data_format(data):
    assert isinstance(data, dict), "Input data should be a JSON object (dictionary)"
    sf_predictions = data['dialogs'][0]['human_utterances'][0]['annotations']['speech_function_predictor']
    assert sf_predictions, 'Speech functions predictions are missing in data'
    hypotheses = [item['text'] for item in data.get('dialogs', [{}])[0].get('human_utterances', [{}])[0].get('hypotheses', [])]
    hypotheses_annotation = [item['annotations']['speech_function_classifier'] for item in data.get('dialogs', [{}])[0].get('human_utterances', [{}])[0].get('hypotheses', [])]
    assert all([hypotheses, hypotheses_annotation]), 'Hypotheses and their annotation with speech functions are missing in the data'
...
def test_output_data_format(data):
    ...
    assert response_text in hypotheses, "Result isn't in hypotheses"
    assert isinstance(response_text, str), "Response text is not a string"
    assert response_text.strip() != "", "Response text is empty"
    print(f"""Test passed: Response is valid and was chosen from hypotheses.
